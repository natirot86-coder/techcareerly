...
-- ─── 0. השלמת עמודות חסרות ב-candidates ─────────────────────────────────────
-- הפרונט (candidate.ts, /api/domain-choice) כותב chosen_domain מאז שלב 3,
-- אבל הסכימה שהוקמה בפועל לא כללה אותה — כלומר שמירת התחום הנבחר נכשלה
-- בשקט. נתגלה 14.8.2026 בהרצת המיגרציה.

alter table public.candidates add column if not exists chosen_domain text;
alter table public.candidates add column if not exists domain_selected_at timestamptz;
