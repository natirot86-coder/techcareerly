# Techcareerly

Web App שמלווה משתתפי טק-קריירה (יוצאי אתיופיה) במסע בחירת מסלול לימודים להייטק.

---

## 🚨 ישראל — קרא את זה לפני כל דבר אחר

**עדכון 10.9.2026.** אנחנו מריצים פיילוט על בוגרי ההכשרות שלנו (ראה
"שני מסעות לקוח" למטה). **הצד שלנו מוכן. שלושה דברים חוסמים, ורובם אצלך.**

### המצב בפרודקשן, עכשיו

```
auth.users     248 · 0 עם טלפון · 0 עם מייל     ← כולם אנונימיים
candidates      54 · כולם cohort=main            ← אף בוגר לא שויך
alumni_roster    0 שורות                         ← ממתין לייצוא ממאנדיי (נתי)
coordinators     2 · סיוון קיימת אבל active=false
cal_bookings     1 · 0 משויכות                   ← בדיוק מה שקורה בלי טלפון
```

### ✅ צ׳קליסט — מה צריך להשלים

**1. מסך טלפון + OTP באונבורדינג** — *ישראל*

היום כל נרשם עובר ב-`signInAnonymously` ולא נשאל למספר אף פעם. הקוד ל-OTP
קיים ועובד ב-`/login`, אבל מגיעים אליו רק דרך קישור "כבר יש לך חשבון?"
בתחתית מסך הפתיחה.

הפונקציות כתובות ב-`src/lib/candidate.ts` — `sendPhoneOtp` ו-`verifyPhoneOtp`,
כולל המרת סשן אנונימי דרך `updateUser({phone})` ואז `verifyOtp` עם
`type: "phone_change"`. **חסר המסך והחיווט לרצף האונבורדינג.**

**2. 019sms כ-Send SMS Hook** — *ישראל*

הקוד מוכן (`/api/auth/send-sms-hook`). חסר:
- Supabase → Auth → Hooks → Send SMS hook, עם ה-URL והסוד
- בוורסל: `SUPABASE_AUTH_HOOK_SECRET` ו-`SMS_019_*`
- Supabase → Auth → Providers → **Phone מסומן פעיל**

⚠️ **1 ו-2 הולכים יחד.** 1 בלי 2 = מסך שמבקש קוד שלא מגיע. 2 בלי 1 = SMS
שאף אחד לא מבקש. **שניהם, או אף אחד.**

**3. `CAL_WEBHOOK_SECRET` בוורסל** — *ישראל*

הקוד מוכן (`/api/cal-webhook`) ואומת בהזמנת אמת מול היומן. חסר רק המשתנה.

**4. להשלים את סיוון בסגל** — *ישראל + נתי*

הרשומה כבר קיימת ב-`coordinators` (`id: sivan-mekonen`) עם **`active=false`**
בכוונה: מדיניות ה-RLS היא `using (active = true)`, ולכן רכזת לא פעילה בלתי
נראית למועמדים ואינה שוברת דבר. ממלאים ב-`/admin/program` והופכים את הדגל:

| שדה | מה |
|---|---|
| `email` | ⚠️ **המייל של חשבון ה-Cal שלה**, לא מייל העבודה |
| `phone` | לוואטסאפ, בפורמט `9725XXXXXXXX` |
| `cal_m1` · `cal_m3` | קישורי היומן. **`cal_m3` הוא "פגישה 2" במסע הבוגרים** |

> **מלכודת המייל:** ה-webhook מזהה לאיזו רכזת שייכת ההזמנה **לפי מייל
> ה-organizer שמגיע מ-Cal**. נפלנו בזה כבר — החשבון של נתי הוא gmail פרטי,
> וכל עוד בסגל היה רשום מייל העבודה, אף הזמנה לא זוהתה.
> (גם אצל נתי `cal_m1`/`cal_m3` ריקים — שווה למלא.)

**5. ה-webhook בחשבון Cal של סיוון** — *ישראל*
אותו URL ואותו סוד משותף. webhook אחד לכל חשבון Cal.

**6. ייצוא הטלפונים ממאנדיי → `alumni_roster`** — *נתי*
טלפון (חובה) · שם · מה למד · מתי סיים. **הזהות קובעת את הקוהורט ולא הקישור:**
מי שנרשם מכל קישור שהוא ומספרו ברשימה מקבל `cohort=alumni` אוטומטית.

### למה זה חוסם — שרשרת, לא רשימה

```
1+2 טלפון  →  6 ייצוא טלפונים  →  4+5 סיוון  →  פיילוט
```

**בלי טלפון:** הזמנות Cal לא מותאמות למועמד · אף בוגר לא מקבל את המסע שלו ·
ומי שמנקה דפדפן מקבל זהות חדשה ומאבד הכל (248 משתמשים מול 54 מועמדים מרמז
שזה כבר קורה).

### בדיקת קבלה כשמסיימים

1. נרשם חדש → מספר מופיע ב-`candidates.phone` מנורמל (`972…`)
2. קביעת פגישה ביומן של סיוון → שורה ב-`cal_bookings` עם `organizer_email`
   שלה ועם `coordinator_id` מזוהה — **כאן מלכודת המייל מתפוצצת אם טעינו**
3. טלפון מ-`alumni_roster` נרשם → מקבל `cohort=alumni` אוטומטית

**חשוב:** הרצת מיגרציות כבר לא דורשת הדבקה ידנית — מ-23.8 הן רצות דרך
ה-Management API. 001–009 כולן רצו.

---|---|
| **התאמת הזמנות Cal למועמד** | הטלפון הוא המפתח המשותף היחיד בין האפליקציה ל-Cal. בלעדיו כל הזמנה נוחתת כ"לא משויכת" |
| **🚨 פיילוט הבוגרים** | `alumni_roster` משייך לפי טלפון. **בלי טלפון אף בוגר לא יקבל את המסע שלו — הפיילוט לא יכול לרוץ** |
| **החלפת מכשיר** | אנונימי שמנקה דפדפן מקבל זהות חדשה ומאבד הכל. 240 מול 46 מרמז שזה כבר קורה |

**מה שצריך:**

1. **מסך טלפון + OTP באונבורדינג.** הפונקציות כתובות ב-`src/lib/candidate.ts`
   (`sendPhoneOtp`, `verifyPhoneOtp`, כולל המרת סשן אנונימי דרך `phone_change`).
   חסר המסך והחיווט לרצף.
2. **019sms כ-Send SMS Hook.** הקוד מוכן (`/api/auth/send-sms-hook`). חסר:
   Supabase → Auth → Hooks, ‎+ `SUPABASE_AUTH_HOOK_SECRET` ו-`SMS_019_*` בוורסל,
   ‎+ Phone provider פעיל ב-Auth → Providers.
3. **`CAL_WEBHOOK_SECRET` בוורסל.** הקוד מוכן ואומת בהזמנת אמת.

⚠️ **הסדר חשוב:** 1 בלי 2 = מסך שמבקש קוד שלא מגיע. 2 בלי 1 = SMS שאף אחד
לא מבקש. **צריך את שניהם יחד**, ורק אז ייצוא הטלפונים ממאנדיי (אצל נתי)
הופך לשימושי.

---

## קישורים מהירים
| | |
|---|---|
| **Production** | https://hasifaapp.vercel.app |
| **GitHub** | https://github.com/natirot86-coder/techcareerly |
| **Design Files** | `design_handoff_tech_career_2026/` |

---

## Stack
| שכבה | כלי | אחראי |
|------|-----|--------|
| Frontend | Next.js 16 + Tailwind v4 + TypeScript | נתי |
| DB + Auth | Supabase | **ישראל** |
| Automation | Make.com | **ישראל** |
| CRM | Monday.com | **ישראל** |
| AI Co-pilot | Claude API | שניהם |

---

## התחלה מהירה (ישראל — קרא את זה)

```bash
git clone https://github.com/natirot86-coder/techcareerly.git
cd techcareerly
npm install
npm run dev
```

פותח על `http://localhost:3000` — תראה את ה-Dashboard עם כל 6 השלבים.

### משתני סביבה
העתק את `.env.local.example` ל-`.env.local` (לא בגיט) ומלא עם הפרטים מה-Supabase project שלך:
```
NEXT_PUBLIC_SUPABASE_URL=...
NEXT_PUBLIC_SUPABASE_ANON_KEY=...
```
בלי הקובץ הזה האפליקציה עובדת ב-mock מקומי (localStorage) כרגיל — שום דבר לא נשבר.

### חיבור Supabase — שלב אחרון (ישראל)
1. צור פרויקט ב-[supabase.com](https://supabase.com) → Settings → API → העתק URL + anon key ל-`.env.local`
2. הרץ את `supabase/schema.sql` ב-SQL Editor של הפרויקט (יוצר את כל הטבלאות + RLS)
3. זהו — הדפים מתחברים אוטומטית (Onboarding, Dashboard, Explore) דרך `src/lib/candidate.ts`

הזהות של המשתמש מבוססת על **Supabase Anonymous Auth** (`auth.uid()` יציב לכל דפדפן) — כך שהנתונים כבר נשמרים אמיתית לפני שיש Phone/OTP. כשתחבר OTP אמיתי, אפשר לשדרג את אותו anonymous user בלי לאבד נתונים (Supabase identity linking).

---

## מבנה הפרויקט

```
src/
├── app/
│   ├── dashboard/page.tsx      # מרכז המסע — השלב נגזר ממה שקרה, לא מכפתור
│   ├── onboarding/page.tsx     # 6 מסכים + סיור (שמות שלבים מ-journey.ts)
│   ├── waiting/page.tsx        # מרחב ההמתנה לפגישה 1: ציר + שתי דקות + הכנה + צ'ק-אין
│   ├── contact/page.tsx        # קביעת פגישות 1/2/3 (Cal.com, בוחר לבד לפי מצב)
│   ├── paths/page.tsx          # שלב 4 — 9 מסכים
│   ├── plan/page.tsx           # שלב 5 — תוכנית, חשבון, ארון מסמכים, עדכון לרכזת
│   ├── reset/page.tsx          # מחיקת מצב מקומי לבדיקה מההתחלה
│   ├── map/                    # 3 תצוגות: תרשים · /flows מסע+צילומים · /grid
│   └── admin/                  # 4 לוחות: institutions · scholarships · courses · analytics
├── data/
│   ├── journey.ts              # מקור אמת יחיד לשמות ששת השלבים (שתי שפות)
│   ├── institutions.ts         # 69 מוסדות — מי שמלמד
│   ├── scholarships.ts         # 29 מלגות ותוכניות — מי שמממן/עוטף + דירוג שקוף
│   ├── courses.ts              # 12 קורסים = מוסד × מעטפת, מחזור נגזר מהתאריך
│   ├── meetings.ts             # שלוש הפגישות + קישורי Cal.com לכל רכז/ת
│   ├── plan.ts                 # מחולל תוכנית שלב 5 (משימות בגודל ישיבה אחת)
│   └── routes.ts / tech-professions.ts / map-flows.ts
├── components/ui/              # JourneyStrip, BottomNav, AllPaths, TrackDetail...
└── lib/candidate.ts            # Supabase: anonymous auth, tasks, simulations

supabase/migrations/001_stage4_stage5_analytics.sql  # ⚠️ ממתין להרצה
public/map-shots/               # צילומי מסך אוטומטיים ל-/map/flows ו-/map/grid
```

---

## שני מסעות לקוח, אפליקציה אחת (1.9.2026)

`candidates.cohort` — ברירת מחדל `main`. **היעדר ערך = ההתנהגות הקיימת**,
ולכן כל מי שכבר במערכת לא הושפע.

| | הקהל הרחב (`main`) | בוגרי טק-קריירה (`alumni`) |
| --- | --- | --- |
| שלבים | 6 | **5** — בלי טעימות |
| פגישות | 3 | **2** — היכרות, ונעילת מסלול |
| מסלולים | תואר · מה״ט · הכשרה | **תואר בלבד** |
| שאלון שלב 4 | 8 שאלות + מנוע משקלים | **6 שאלות, בלי מנוע** |

**מה מתפצל: השאלון בלבד** (`src/app/paths/AlumniIntake.tsx`). מוסדות, תארים,
מלגות, חסמים, חקר, שלב 5 ואישור הלימודים — **אותו קוד בדיוק**. הכלל: מכפילים
מה שגלוי וזול לטעות בו, לעולם לא מה ששקט ויקר.

**החוזה שמונע התנגשות:** שאלון הבוגרים כותב את אותו `paths-quiz` — אותם
מפתחות ואותם ערכי `A`/`B`/`C` — כי שלב 5 גוזר ממנו את שעות הפנאי לחשבון
המלגות ובונה ממנו את רשימת המשימות. `WEIGHTS` לא נוגעים בו: 52/10/38 קפוא.

📄 המפרט המלא: `docs/pilot-alumni-spec.md`

**קישורים לשיתוף עם הרכזת:**

| | |
|---|---|
| מערך פגישה 1 | https://hasifaapp.vercel.app/guides/meeting1.html |
| מה שהמשתתף קורא לפני הפגישה | `/waiting?demo=1&cohort=alumni` |
| השאלון והמוסדות | `/paths?demo=1&cohort=alumni` |
| המסע שלו · שאלות · מלגות | `/journey` · `/faq` · `/plan` + אותם פרמטרים |

⚠️ `?demo=1&cohort=alumni` היא **תצוגה בלבד — לא נכתב דבר**, לא ב-localStorage
ולא ב-DB, והיא נפתחת בלי פגישה קבועה. מי שנשלח לו הקישור לא נספר כמועמד.

⚠️ **מקור אחד למערך:** `docs/pilot-alumni-meeting1.html`. `public/guides/meeting1.html`
נוצר ממנו — לא עורכים את הציבורי.
🗺 במפת האפליקציה (`/map`) יש **בורר מסע** שמראה מה משותף ומה בלעדי.

---

## מסכים — סטטוס (1.9.2026)

| מסך | סטטוס | הערות |
|-----|--------|-------|
| Onboarding (6 מסכים + סיור) | ✅ | RTL, חזרה בסיור, טקסט כן ("אתה קובע — לא מחכים לשיחה") |
| **מרחב ההמתנה** (`/waiting`) | ✅ | שתי הדקות נעולות עד קביעה · צ'ק-אין "איך היה?" · at-risk נאסף |
| קביעת פגישות (`/contact`) | ✅ | 3 פגישות, הדף בוחר לבד; פגישה 1 נוחתת ישר במרחב ההמתנה |
| שלב 3 — חקר תחומים | ✅ | כל 9 התחומים מלאים (sim + day + mystery + experience) |
| **שלב 4** (`/paths`) | ✅ | 9 מסכים · שער נגזר · שאלון בוגרים נפרד |
| **שלב 5** (`/plan`) | ✅ | תוכנית לפי חודשים, חשבון, ארון (בלי קבצים!), עדכון לרכזת |
| **4 לוחות ניהול** (`/admin/*`) | ✅ | אישור פרטני, תצוגת קריאה, פס "דורש טיפול", ייצוא JSON |
| **3 תצוגות מפה** (`/map*`) | ✅ | תרשים · מסע עם צילומים והסבר · גריד. נתי בוחר אחת |
| איפוס (`/reset`) | ✅ | לבדיקת המסע מההתחלה |
| חיבור שלבים 4–5 לסופאבייס | ✅ | המיגרציה רצה 14.8 |
| **FAQ** (`/faq`) | ✅ | במקום צ'אט ה-AI — תשובות ידניות, ממוגדרות, מודעות-קוהורט |
| **קהילה** (`/squad`) | ✅ | אירועים · קבוצות · בקשה לדבר עם בוגר. ⏳ חסרים הקישורים |
| **פיילוט הבוגרים** | ✅ | תשתית מלאה. ⏳ חסום על ייצוא הטלפונים ממאנדיי |

---

## מה צריך מישראל (Backend)

> ✅ **משתני הסביבה תוקנו ב-12.8.2026** — היו שמורים עם BOM ו-`\r\n`, החיבור אומת ועובד.

> ✅ מיגרציות 001–009 רצו כולן. מ-23.8 יש לקלוד Supabase Access Token והן
> רצות דרך ה-Management API — **אין יותר "הדבק SQL בדשבורד"**.

1. **Phone Auth בפרודקשן** — לוודא שהוא פעיל. זה מה שמפעיל את שכבת הזיהוי
   האוטומטית בין המועמד להזמנת Cal (הטלפון הוא המפתח המשותף היחיד)
2. **`CAL_WEBHOOK_SECRET`** ב-Vercel — הקוד מוכן (`/api/cal-webhook`)
3. **חיבור 019sms כ-Send SMS Hook** — הקוד מוכן (`/api/auth/send-sms-hook`);
   חסר ב-Supabase Dashboard → Auth → Hooks, ‎+ `SUPABASE_AUTH_HOOK_SECRET`
   ו-`SMS_019_*` בוורסל
4. **`chat_messages` ו-`nudges`** — קיימות בבסיס ולא מחוברות. `chat_messages`
   ככל הנראה מיותרת מאז שה-AI Chat הוחלף ב-FAQ ידני

## לוג עדכונים

### 2026-09-01 — שני מסעות לקוח · QA סתירות · בורר מסע במפה

- **פיילוט האקדמיה לבוגרים חי בתשתית.** `cohort` ב-DB (מיגרציה 009) +
  `alumni_roster`. **הזהות קובעת את השיוך ולא הקישור** — קישור עובר בוואטסאפ
  ותמיד ידלוף; טלפון ברשימה משייך נכון מכל כניסה שהיא.
- **`journey.ts` מודע-קוהורט** — `alumni` מוגדר כ**חיסור** מ-main, לא כרשימה
  שנייה, כדי שלא תתיישן. לכל שלב `id` יציב ו-`stageNumFor` מתרגם מספרים.
- **השער נגזר:** `gateMeetingFor` מחזירה את הפגישה שסוגרת את השלב הקודם —
  2 לקהל הרחב, **1 לבוגרים**. שער עם "2" מוקלד היה נועל כל בוגר מאחורי מסך ריק.
- **QA סתירות:** ארבעה מקומות סתרו את מסע הבוגרים — האונבורדינג ("6 שלבים"),
  שלב 5 ("שלב 5 מתוך 6"), ה-FAQ (שישה שלבים, שלוש פגישות, שאלה על טעימות
  שאין להם) ו"שאלות לפגישה 3". כולם נגזרים עכשיו.
- **באג שנתפס בדרך:** לסרפנטינה של מסך הרכזת היו 13 תחנות ו-12 דגלים —
  תחנת המסלול נוספה ב-28.8 בלי הדגל שלה, וכל תחנה אחריה הציגה את מצב שכנתה.
  `done` עבר לתוך הגדרת התחנה.
- **אנליטיקות מפולחות לפי קוהורט** — ברירת מחדל `main`, כדי שהמספר הקיים
  ימשיך לומר בדיוק מה שאמר. בוצע **לפני** שנכנס מועמד ראשון.
- **`/map` קיבל בורר מסע** — אותו קנבס, הדגשה אחרת. 88 מסכים · הקהל הרחב 87 ·
  בוגרים 36.


### 2026-08-24 — כל 9 התחומים מלאים · מבוא להייטק · מסך תוצאה 1a · Cal חי

- **טעימות ai/ux/marketing הושלמו** — day + מיני-פרויקט (מרפאה/גמ"ח/מספרה,
  החלטת נתי: בונים ולא מפענחים) + SCCT. מפות מסע נעולות דרך `TasteJourney`
  המשותפת. כל 9 התחומים מלאים.
- **מבוא לעולם ההייטק** החליף את סימולציית ה-AI במרחב ההמתנה — 7 כרטיסים
  בעקרון ניחוש-לפני-חשיפה, כל מספר מאומת עם מקור מוצג (רשות החדשנות 2026,
  למ"ס), שרשרת המוצר דרך Waze, AI על כל התעשייה, סיפורי בוגרים מ-ynet.
- **מסך תוצאת שלב 4 בעיצוב 1a** — רשת שוויונית עם כתר, 3 שורות השוואה
  קבועות, בחירה לא-מומלצת נמדדת (`track_choice`).
- **Cal.com רב-רכזות חי** — webhook + סוד בוורסל, אומת בהזמנת אמת; זיהוי
  רכזת לפי מייל ה-organizer; קישורי יומן פר-רכזת בסגל (מיגרציה 006).
- שעון זכאות משוחררים: מילואים פעיל = 10 שנים. 6 קורסי מחקר כישויות.

### 2026-08-11 — שלב 4 נבנה מחדש (מסלולי לימוד)

**מנוע ההמלצה**
- וטו-OR הוחלף בניקוד משוקלל. התואר מקבל יתרון בסיס וכל תיקו.
  אומת על כל 729 צירופי התשובות: **תואר עלה מ-5% ל-52%**, מה"ט 10%, הכשרה 38%
- שאלת הזמן נוסחה מחדש — מ"מתי אתה רוצה לעבוד" (משאלה שכולם עונים עליה אותו דבר)
  ל**רשת ביטחון כלכלית**, שהיא מגבלה אמיתית
- "ילדים" ו"מיקום" משפיעים על ההמלצה. קודם הן היו דקורטיביות

**מסכים חדשים** (`/paths`)
- **מסך החסמים** — כל מגבלה שהמשתמש הודה בה מקבלת מענה עם שם ותאריך שמחושב מול היום
- **כל הדרכים מכאן** — שלושה מסלולים כקווי רכבת, אורך הקו מייצג משך. רגע ההכנסה
  הוא הגיבור: הקו הופך ירוק ממנו ומטה. עיצוב 7a מ-Claude Design
- **מסך עומק לכל מסלול** — איך זה נכנס לשבוע, קבלה, מימון, מה קורה כשקשה, לאן זה מוביל
- **ערכת חקר** (אופציונלי) — למי לפנות, עם חיסון מראש מפני "אתה לא עומד בתנאים"
- **סיכום** — מפריד "לפני הפגישה" (מועדים שלא מחכים) מ"בפגישה", CTA מודע-מצב,
  ויציאה כנה של "אני צריך לחשוב"

**תוכן**
- רשימת המוסדות נכתבה מחדש ממחקר מאומת. אריאל הוסתרה (אין תשתית תמיכה לקהילה),
  קמפוס IL הוסתר (כלי העשרה, לא מסלול)
- האוניברסיטה הפתוחה עם אזהרה אדומה: 33% לא מגיעים לשנה ב׳ מול פחות מ-5% במקומות אחרים
- הוסרה קרן שלא קיימת. הוחלפה בתוכניות מאומתות
- מה"ט הועבר למקום האחרון וממוסגר לקריירה ביטחונית/ממשלתית/חומרה
- שדה `domains` לכל מוסד — UX מקבל את שנקר, לא את בן-גוריון

**תשתית**
- `src/data/institutions.ts` כמקור אמת יחיד · `/admin/institutions` לעריכה וייצוא
- `JourneyStrip` — מה הושלם / איפה אני / מה נשאר
- Vercel Analytics עם אירועי משפך, כולל נטישה ברזולוציית שאלה
- ניווט עליון לדסקטופ — קודם הניווט כולו היה ב-BottomNav שמוסתר ב-md
- `?reset=1` ו-`?demo=1&phase=X` לסקירה בלי לעבור את כל הזרימה

### 2026-07-19 — Supabase foundation
- ✅ `@supabase/supabase-js` הותקן
- ✅ `supabase/schema.sql` — candidates, tasks, domain_rankings, simulation_progress, chat_messages, nudges + RLS מלא
- ✅ `src/lib/supabase.ts` + `src/lib/candidate.ts` — client + helpers, כולל Anonymous Auth
- ✅ Onboarding, Dashboard, Explore מחוברים ל-Supabase (fallback ל-mock אם אין `.env.local`)
- ⏳ נשאר לישראל: יצירת הפרויקט האמיתי + הרצת ה-schema (ה-CLI לא זמין כאן, צריך supabase.com)

### 2026-07-14 — סבב 3
- ✅ GitHub repo הוקם: `natirot86-coder/techcareerly`
- ✅ Vercel production: https://hasifaapp.vercel.app
- ✅ ישראל (Israelman) הוזמן כ-collaborator
- ✅ README מלא + מסמכים מעודכנים

### 2026-07-14 — סבב 2
- ✅ Tailwind v4 design tokens (navy, orange, cream)
- ✅ Google Fonts: Noto Sans Hebrew + Noto Serif Hebrew
- ✅ קומפוננטות: MonogramBadge, Button, TaskCard, ProgressDots, NavyHeader, BottomNav
- ✅ Dashboard: כל 6 שלבי המסע עם mock state
- ✅ Viewport מוגדר למובייל (iOS + אנדרואיד)

### 2026-07-14 — סבב 1
- ✅ Next.js 16 + Tailwind v4 + TypeScript
- ✅ CLAUDE.md, PRD.md, docs/architecture.md
- ✅ design handoff מ-Claude Design

---

## החלטות שנקבעו
| נושא | החלטה |
|------|--------|
| Framework | Next.js 16, App Router |
| CSS | Tailwind v4 — tokens ב-`globals.css` |
| Backend | ישראל בלבד — Supabase |
| State כרגע | Supabase (Anonymous Auth) עם fallback ל-mock מקומי כשאין `.env.local` |
| ניווט | מובייל: Bottom Nav · דסקטופ: סרגל עליון. טאב "חקר" מודע-הקשר (→ `/paths` אחרי פגישה 2) |
| עמדה | **כלי עמדה, לא ניטרלי** — ממליצים על תואר במפורש, עם המקרים הכנים שבהם הוא לא נכון |
| נתוני מוסדות | מה שלא אומת מסומן `needs-check`. לא ממציאים טלפונים או סכומים |
| שם | techcareerly (זמני) |
| Demo user | נועה |
| Mobile | Mobile-first, max-width 390px, RTL (dir="rtl") |
| Deployment | Vercel — auto-deploy מכל push ל-master |
