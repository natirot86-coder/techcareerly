...
**אין לאף משתמש טלפון, וזה מפיל שלושה דברים שכבר בנויים.**

```
auth.users:  240 משתמשים · 0 עם טלפון · 0 עם מייל
candidates:   46 · 0 עם טלפון
```

כולם אנונימיים, ומשתי סיבות: **האונבורדינג לא מבקש טלפון בכלל** (הקוד ל-OTP
קיים ב-`/login`, אבל מגיעים אליו רק דרך קישור "כבר יש לך חשבון?" בתחתית מסך
הפתיחה — נרשם חדש עובר ב-`signInAnonymously` ולא נשאל אף פעם), **ואין SMS
שיגיע** בלי חיבור 019sms.
