...
const NEXT: Record<MeetingNum, { stage: string; title: string; body: string; href: string; cta: string }> = {
  // מפנה למרחב ההמתנה ולא ל-/explore. שם יש **טעימה אחת**, וזו המשמעת שמונעת
  // ממנו להגיע לפגישה מעוגן בתחום שבחר לבד. שבעת התחומים נפתחים בשלב 3.
  1: {
    stage: "עד הפגישה",
    title: "בינתיים אפשר לטעום",
    body: "יש שם טעימה אחת קצרה — כמה דקות שנותנות תחושה איך עולם אחד נראה מבפנים. אין ציון, אי אפשר להיכשל, ואף אחד לא סופר את זה.",
    href: "/waiting",
    cta: "למרחב ההמתנה ←",
  },
  2: {
    stage: "השלב הבא · שלב 4",
    title: "לא חייבים לחכות לפגישה",
  ...
  3: {
    stage: "השלב הבא · שלב 5",
    title: "אפשר להתחיל לסדר את הדרך",
...
export default function BookedPage() {
  ...
  useEffect(() => {
    ...
    if (param && ["1", "2", "3"].includes(param)) setM(Number(param) as MeetingNum);
