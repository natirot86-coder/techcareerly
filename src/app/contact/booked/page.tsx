...
/**
 * מה יקרה בפגישה — ממוגדר (נתי 27.8): המגדר ידוע מהאונבורדינג ופונים בו.
 * הצורה המשולבת נשארת רק כשאין עדיין מגדר שמור. מחושב בקומפוננטה (לא
 * במודול) כדי שלא יהיה פער בין רינדור שרת ללקוח.
 */
function whatHappensFor(m: MeetingNum, g: string | null): { emoji: string; text: string }[] {
  const pick = (male: string, female: string, other: string) =>
    g === "male" ? male : g === "female" ? female : other;
  const ALL: Record<MeetingNum, { emoji: string; text: string }[]> = {
    1: [
      { emoji: "☕", text: pick(
        "נכיר. הרכזת תשאל מאיפה אתה מגיע ומה מביא אותך לכאן",
        "נכיר. הרכזת תשאל מאיפה את מגיעה ומה מביא אותך לכאן",
        "נכיר. הרכזת תשאל מאיפה את/ה מגיע/ה ומה מביא אותך לכאן") },
      { emoji: "🗺️", text: "נבנה יחד תוכנית עבודה — מה הצעדים, ובאיזה קצב" },
      { emoji: "🔍", text: pick(
        "בסוף ייפתח לך חקר תחומי ההייטק, ותוכל להתחיל לטעום",
        "בסוף ייפתח לך חקר תחומי ההייטק, ותוכלי להתחיל לטעום",
        "בסוף ייפתח לך חקר תחומי ההייטק, ותוכל/י להתחיל לטעום") },
    ],
    2: [
      { emoji: "🎯", text: "הרכזת תקרא את הסיכום שהכנת — הפגישה מתחילה כבר ממקום מעמיק" },
      { emoji: "🔍", text: pick(
        "תעברו על הטעימות שעשית — מה אהבת, מה פחות, איפה אתה מרגיש שתצליח ולמה",
        "תעברו על הטעימות שעשית — מה אהבת, מה פחות, איפה את מרגישה שתצליחי ולמה",
        "תעברו על הטעימות שעשית — מה אהבת, מה פחות, איפה את/ה מרגיש/ה שתצליח ולמה") },
      { emoji: "🗺️", text: pick(
        "בסוף הפגישה תבחר תחום — אחרי שהתייעצת עם הרכזת. ואז היא תפתח איתך את השלב הבא: חקר מסלולי הלימוד, למצוא את המסלול הנכון לתחום שבחרת",
        "בסוף הפגישה תבחרי תחום — אחרי שהתייעצת עם הרכזת. ואז היא תפתח איתך את השלב הבא: חקר מסלולי הלימוד, למצוא את המסלול הנכון לתחום שבחרת",
        "בסוף הפגישה תבחר/י תחום — אחרי שהתייעצת עם הרכזת. ואז היא תפתח איתך את השלב הבא: חקר מסלולי הלימוד, למצוא את המסלול הנכון לתחום שבחרת") },
    ],
    3: [
      { emoji: "🎓", text: "תעברו יחד על המוסדות שברשימה ועל השאלות שהכנת" },
      { emoji: "✅", text: pick(
        "תבחר את ההחלטה הסופית — איזה מסלול ובאיזה מוסד — בליווי הרכזת",
        "תבחרי את ההחלטה הסופית — איזה מסלול ובאיזה מוסד — בליווי הרכזת",
        "תבחר/י את ההחלטה הסופית — איזה מסלול ובאיזה מוסד — בליווי הרכזת") },
...
const NEXT: Record<MeetingNum, { stage: string; title: string; body: string; href: string; cta: string }> = {
  ...
  3: {
    ...
    body: "למלגות יש תאריכים אחרונים וחלקם קרובים — לא כדאי לחכות לפגישה. כבר עכשיו אפשר לראות מה נסגר מתי, כמה זה עולה אחרי המלגות ואילו מסמכים להכין. את ההרשמה עצמה סוגרים יחד בפגישה.",
    href: "/plan",
    cta: "לתוכנית שלי ←",
  },
};
...
export default function BookedPage() {
  ...
  useEffect(() => { try { setGender(localStorage.getItem("user-gender")); } catch { /* ignore */ } }, []);
  const points = whatHappensFor(m, gender);
  const next = NEXT[m];
