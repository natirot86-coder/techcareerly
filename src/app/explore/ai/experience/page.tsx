...
const CONSTRUCTS = [
  {
    ...
    color: AI,
    emoji: "🤖",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה עולם ה-AI ויישומו בעסקים מדבר אליך?",
  ...
  },
  {
    ...
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה הרגשת שאת יכולה ללמוד ולהצליח בתחום?"
      : "בסולם 1–5 — כמה הרגשת שאתה יכול ללמוד ולהצליח בתחום?",
    scaleHint: (g: Gender) => g === "female"
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך בונה עוזרי AI לעסקים?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך בונה עוזרי AI לעסקים?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  const constructs = [
    { label: "עניין",   score: interestScore, color: AI,     emoji: "🤖", desc: "כמה התחום מדבר אליך" },
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי לעולם ה-AI";
    subtext = "שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. זה דיוק נדיר. כדאי לשוחח עם הרכזת על מסלול שמוביל לעבודה עם AI.";
    ...
    subtext = "עולם ה-AI הוא רק אחת מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="ai" domainLabel="ה-AI" />
          ...
          {[
            {
              ...
              title: "קורסי הייטק ובינה מלאכותית בעברית — פלטפורמת הלמידה הממשלתית",
              lang: "עברית",
            ...
            },
            {
              ...
              title: "מבוא לבינה מלאכותית — הקורס החינמי של אוניברסיטת הלסינקי",
              lang: "אנגלית",
            ...
            },
            {
              ...
              title: "מדריך חינמי לכתיבת פרומפטים — מהצעד הראשון",
              lang: "אנגלית",
            ...
            >
              <div className="flex items-center justify-center" style={{ aspectRatio: "16/9", background: "rgba(124,58,237,0.08)" }}>
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1.5">
                    ...
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: `${AI}12`, color: AI }}>{c.lang}</span>
                  </div>
                  <span className="text-[11px] font-bold" style={{ color: AI }}>לקורס ←</span>
                </div>
              </div>
            </a>
          ))}
...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("ai-experience-state");
...
export default function AiExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("ai-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  }, [phase, gender, answers]);

  /*
   * איפה עוצרים בתוך הכלי — אותו רציונל כמו ברשתות: בלי scct_step,
   * מי שענה על ארבע שאלות מתוך שש ונעלם פשוט לא קיים בנתונים.
   */
  useEffect(() => {
    if (phase === "gender" || phase === "done") return;
    logEvent("scct_step", { domain: "ai", q: phase });
  }, [phase]);
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("ai-experience", JSON.stringify(newAnswers));
        /*
         * מדד השליחות נשמר לסופאבייס: עניין גבוה + מסוגלות נמוכה הוא בדיוק
         * האדם שבשבילו הארגון קיים. הסקאלות בכלי הן 1–5 — מה ש-scct_scores מצפה לו.
         */
        saveScctScore(
          "ai",
        ...
        logEvent("scct_done", { domain: "ai" });
        const journey = JSON.parse(localStorage.getItem("ai-journey") || "{}");
        localStorage.setItem("ai-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  if (phase === "gender") {
    return (
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: AI }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.75 }}>כלי עיבוד החוויה · AI ובינה מלאכותית</div>
        ...
        <div className="max-w-[720px] mx-auto px-[22px] pt-6 pb-36">
          <div className="rounded-2xl p-4 mb-6" style={{ background: "rgba(124,58,237,0.05)", border: "1.5px solid rgba(124,58,237,0.15)" }}>
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              style={{ background: AI, color: "#fff", ...HEEBO }}
            >
              אליה — בלשון נקבה
            </button>
            <button
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/ai" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
