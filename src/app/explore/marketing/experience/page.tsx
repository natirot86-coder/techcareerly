...
const CONSTRUCTS = [
  {
    ...
    color: MKT,
    emoji: "📣",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה שיווק דיגיטלי מדבר אליך?",
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך עובדת בשיווק דיגיטלי?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך עובד בשיווק דיגיטלי?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  const constructs = [
    { label: "עניין",   score: interestScore, color: MKT,    emoji: "📣", desc: "כמה התחום מדבר אליך" },
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי לשיווק דיגיטלי";
    subtext = "שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. כדאי לשוחח עם הרכזת על מסלולי כניסה לתחום ועל איך בונים קמפיין ראשון להצגה.";
    ...
    subtext = "שיווק דיגיטלי הוא רק אחת מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="marketing" domainLabel="השיווק הדיגיטלי" />
    </div>
  );
}
...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("marketing-experience-state");
...
export default function MarketingExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("marketing-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  }, [phase, gender, answers]);

  // איפה עוצרים בתוך הכלי — כל מעבר שאלה נרשם, כמו בשאר התחומים
  useEffect(() => {
    if (phase === "gender" || phase === "done") return;
    logEvent("scct_step", { domain: "marketing", q: phase });
  }, [phase]);
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("marketing-experience", JSON.stringify(newAnswers));
        ...
        logEvent("scct_done", { domain: "marketing" });
        const journey = JSON.parse(localStorage.getItem("marketing-journey") || "{}");
        localStorage.setItem("marketing-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  if (phase === "gender") {
    return (
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: MKT }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.75 }}>כלי עיבוד החוויה · שיווק דיגיטלי</div>
        ...
        <div className="max-w-[720px] mx-auto px-[22px] pt-6 pb-36">
          <div className="rounded-2xl p-4 mb-6" style={{ background: "rgba(249,115,22,0.05)", border: "1.5px solid rgba(249,115,22,0.15)" }}>
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              style={{ background: MKT, color: "#fff", ...HEEBO }}
            >
              אליה — בלשון נקבה
            </button>
            <button
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/marketing" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
