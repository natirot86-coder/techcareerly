...
const CONSTRUCTS = [
  {
    ...
    color: BLUE,
    emoji: "💻",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה פיתוח תוכנה מדבר אליך?",
  ...
  },
  {
    ...
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה הרגשת שאת יכולה ללמוד ולהצליח בפיתוח תוכנה?"
      : "בסולם 1–5 — כמה הרגשת שאתה יכול ללמוד ולהצליח בפיתוח תוכנה?",
    scaleHint: (g: Gender) => g === "female"
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך מצליחה כמפתחת?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך מצליח כמפתח?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי לפיתוח תוכנה";
    subtext = `שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. זה דיוק נדיר. כדאי לשוחח עם הרכזת על מסלול פיתוח מעמיק.`;
    ...
    subtext = "תחום פיתוח התוכנה הוא רק אחד מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="code" domainLabel="הקוד" />
      ...
      <div style={{ borderTop: "1px solid rgba(0,0,0,0.07)" }} className="pt-7">
        ...
        <div className="text-[14px] font-bold mb-4" style={{ color: NAVY }}>
          קורסי פיתוח תוכנה חינמיים
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            {
              img: "/courses/code-freecodecamp.png",
              platform: "freeCodeCamp",
              title: "Full Stack — תוכנית לימודים מלאה בחינם",
              lang: "אנגלית",
              href: "https://www.freecodecamp.org/learn",
            },
            {
              img: "/courses/code-meta.jpg",
            ...
            },
            {
              ...
              title: "קורסי תכנות בעברית — פלטפורמת הלמידה הממשלתית",
              lang: "עברית",
...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("code-experience-state");
...
export default function ExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("code-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("code-experience", JSON.stringify(newAnswers));
        const journey = JSON.parse(localStorage.getItem("code-journey") || "{}");
        localStorage.setItem("code-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: NAVY }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.55 }}>כלי עיבוד החוויה · פיתוח תוכנה</div>
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/code" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
