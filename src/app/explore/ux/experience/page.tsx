...
const CONSTRUCTS = [
  {
    ...
    color: PINK,
    emoji: "🎨",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה עיצוב מוצר ו-UX מדברים אליך?",
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך עובדת כמעצבת מוצר?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך עובד כמעצב מוצר?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  const constructs = [
    { label: "עניין",   score: interestScore, color: PINK,   emoji: "🎨", desc: "כמה התחום מדבר אליך" },
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי לעיצוב מוצר";
    subtext = "שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. כדאי לשוחח עם הרכזת על מסלול עיצוב UX/UI, כולל מה נדרש כדי לבנות תיק עבודות — כי בתחום הזה התיק הוא שער הכניסה.";
    ...
    subtext = "תחום העיצוב הוא רק אחת מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="ux" domainLabel="ה-UX" />
    </div>
  );
}
...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("ux-experience-state");
    return saved ? JSON.parse(saved) : {};
  } catch { return {}; }
}

export default function UxExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("ux-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  }, [phase, gender, answers]);

  // איפה עוצרים בתוך הכלי — כל מעבר שאלה נרשם, כמו בשאר התחומים
  useEffect(() => {
    if (phase === "gender" || phase === "done") return;
    logEvent("scct_step", { domain: "ux", q: phase });
  }, [phase]);
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("ux-experience", JSON.stringify(newAnswers));
        // הסקאלות 1–5 נשמרות לסופאבייס — בדיוק מה ש-scct_scores מצפה לו
        saveScctScore(
          "ux",
        ...
        logEvent("scct_done", { domain: "ux" });
        const journey = JSON.parse(localStorage.getItem("ux-journey") || "{}");
        localStorage.setItem("ux-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  if (phase === "gender") {
    return (
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: PINK }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.75 }}>כלי עיבוד החוויה · עיצוב UX/UI</div>
        ...
        <div className="max-w-[720px] mx-auto px-[22px] pt-6 pb-36">
          <div className="rounded-2xl p-4 mb-6" style={{ background: "rgba(219,39,119,0.05)", border: "1.5px solid rgba(219,39,119,0.15)" }}>
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              style={{ background: PINK, color: "#fff", ...HEEBO }}
            >
              אליה — בלשון נקבה
            </button>
            <button
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/ux" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
