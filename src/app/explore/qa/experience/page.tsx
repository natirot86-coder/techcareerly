...
const CONSTRUCTS = [
  {
    ...
    color: AMBER,
    emoji: "🐞",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה בדיקות תוכנה (QA) מדברות אליך?",
  ...
  },
  {
    ...
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה הרגשת שאת יכולה ללמוד ולהצליח ב-QA?"
      : "בסולם 1–5 — כמה הרגשת שאתה יכול ללמוד ולהצליח ב-QA?",
    scaleHint: (g: Gender) => g === "female"
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך מצליחה כ-QA Engineer?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך מצליח כ-QA Engineer?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  const constructs = [
    { label: "עניין",   score: interestScore, color: AMBER,  emoji: "🐞", desc: "כמה התחום מדבר אליך" },
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי ל-QA";
    subtext = `שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. זה דיוק נדיר. כדאי לשוחח עם הרכזת על מסלול QA מעמיק.`;
    ...
    subtext = "תחום ה-QA הוא רק אחד מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="qa" domainLabel="ה-QA" />
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1.5">
                    ...
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: `${AMBER}12`, color: AMBER }}>{c.lang}</span>
                  </div>
                  <span className="text-[11px] font-bold" style={{ color: AMBER }}>לקורס ←</span>
                </div>
              </div>
            </a>
          ))}
...
export default function ExperiencePage() {
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("qa-experience", JSON.stringify(newAnswers));
        const journey = JSON.parse(localStorage.getItem("qa-journey") || "{}");
        localStorage.setItem("qa-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: NAVY }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.55 }}>כלי עיבוד החוויה · בדיקות תוכנה</div>
        ...
        <div className="max-w-[720px] mx-auto px-[22px] pt-6 pb-36">
          <div className="rounded-2xl p-4 mb-6" style={{ background: "rgba(217,119,6,0.05)", border: "1.5px solid rgba(217,119,6,0.15)" }}>
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/qa" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
