...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("networks-experience-state");
...
export default function NetworksExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("networks-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  ...
  useEffect(() => {
    ...
    logEvent("scct_step", { domain: "networks", q: phase });
  }, [phase]);
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        ...
        logEvent("scct_done", { domain: "networks" });
