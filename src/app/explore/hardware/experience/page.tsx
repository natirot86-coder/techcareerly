...
const CONSTRUCTS = [
  {
    ...
    color: HW,
    emoji: "🔌",
    scaleQ: (_g: Gender) => "בסולם 1–5 — כמה חומרה ואלקטרוניקה מדברים אליך?",
  ...
  },
  {
    ...
    emoji: "🔭",
    scaleQ: (g: Gender) => g === "female"
      ? "בסולם 1–5 — כמה את מדמיינת את עצמך עובדת עם חומרה ואלקטרוניקה?"
      : "בסולם 1–5 — כמה אתה מדמיין את עצמך עובד עם חומרה ואלקטרוניקה?",
    scaleHint: (g: Gender) => g === "female"
...
function Summary({ answers, gender }: { answers: Answers; gender: Gender | null }) {
  ...
  const constructs = [
    { label: "עניין",   score: interestScore, color: HW,     emoji: "🔌", desc: "כמה התחום מדבר אליך" },
  ...
  if (avg >= 4) {
    heroEmoji = "🎯";
    headline = "יש לך חיבור אמיתי לחומרה";
    subtext = "שלושת הסימנים — עניין, ביטחון, ותמונה עתידית — נדלקו. זה דיוק נדיר. כדאי לשוחח עם הרכזת על מסלול הנדסת חשמל או הנדסאי אלקטרוניקה.";
    ...
    subtext = "תחום החומרה הוא רק אחת מהאפשרויות. הרכזת תעזור להבין מה מתאים לך יותר — לפעמים הכיוון הנכון נמצא בתחום אחר.";
  }
  ...
  return (
    <div className="px-[22px] pt-6 pb-32">
      ...
      <ExperienceCTAs domainId="hardware" domainLabel="החומרה" />
    </div>
  );
}
...
function loadSavedState(): { phase?: Phase; gender?: Gender | null; answers?: Answers } {
  ...
  try {
    const saved = localStorage.getItem("hardware-experience-state");
    return saved ? JSON.parse(saved) : {};
  } catch { return {}; }
}

export default function HardwareExperiencePage() {
  ...
  useEffect(() => {
    try { localStorage.setItem("hardware-experience-state", JSON.stringify({ phase, gender, answers })); } catch {/* ignore */}
  }, [phase, gender, answers]);

  // איפה עוצרים בתוך הכלי — כל מעבר שאלה נרשם, כמו בשאר התחומים
  useEffect(() => {
    if (phase === "gender" || phase === "done") return;
    logEvent("scct_step", { domain: "hardware", q: phase });
  }, [phase]);
  ...
  function handleOpen(val: string) {
    ...
    if (construct.id === "interest") {
      ...
      try {
        localStorage.setItem("hardware-experience", JSON.stringify(newAnswers));
        // מדד השליחות נשמר לסופאבייס — הסקאלות 1–5, בדיוק מה ש-scct_scores מצפה לו
        saveScctScore(
          "hardware",
        ...
        logEvent("scct_done", { domain: "hardware" });
        const journey = JSON.parse(localStorage.getItem("hardware-journey") || "{}");
        localStorage.setItem("hardware-journey", JSON.stringify({ ...journey, experience: true }));
      } catch {/* ignore */}
      setPhase("done");
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  if (phase === "gender") {
    return (
      <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
        <div className="text-white px-[22px] pt-[26px] pb-[30px]" style={{ background: HW }}>
          <div className="max-w-[720px] mx-auto">
            <div className="text-[12px] mb-2" style={{ opacity: 0.75 }}>כלי עיבוד החוויה · חומרה ואלקטרוניקה</div>
        ...
        <div className="max-w-[720px] mx-auto px-[22px] pt-6 pb-36">
          <div className="rounded-2xl p-4 mb-6" style={{ background: "rgba(124,45,18,0.05)", border: "1.5px solid rgba(124,45,18,0.15)" }}>
            <div className="text-[13.5px] leading-[1.65]" style={{ color: "rgba(0,0,0,0.65)" }}>
              ...
              שש שאלות קצרות יעזרו לך להבין מה זה אמר לך — על העניין שלך, על הביטחון שלך, ועל האופן שבו את רואה את עצמך בתחום.
            </div>
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              style={{ background: HW, color: "#fff", ...HEEBO }}
            >
              אליה — בלשון נקבה
            </button>
            <button
  ...
  return (
    <div className="min-h-screen" style={{ background: "#fbf9f5" }} dir="rtl">
      ...
      <div className="text-white px-[22px] pt-[26px] pb-7" style={{ background: construct.color }}>
        <div className="max-w-[720px] mx-auto">
          <Link href="/explore/hardware" className="text-[12px] font-bold block mb-5" style={{ opacity: 0.75 }}>
            ← חזרה
          </Link>
              ...
              {[1, 2, 3, 4, 5].map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 text-right">
                    <span className="text-[14px] font-bold" style={{ color: tapped === n ? construct.color : NAVY }}>
                      {n} — {SCALE_LABELS[n]}
                    </span>
                  </div>
                  <div
