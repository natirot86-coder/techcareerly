...
/**
 * מה שאפשר לסמן ב"מה יש לך ביד" — **מקור אמת אחד לכל הקוהורטים** (1.9).
 *
 * הכל בניסוח חיובי: "מה יש לך", ולא "מה חסר לך". כל הסעיפים כאן מזינים את
 * חישוב תנאי הכניסה ואת מסך החסמים, ולכן רשימה מקוצרת בזרימה אחרת הייתה
 * גורמת לשתי הזרימות לחשב אחרת על אותו אדם בדיוק.
 *
 * ⚠️ **הציון בפסיכומטרי לא נשאל בכוונה** — מספר על מסך מזמין שיפוט עצמי,
 * ואצל מי שתחושת המסוגלות שלו נמוכה זו נקודת הנטישה. שואלים רק אם הוא מרוצה.
 */
export const HAVE_CHIPS: { id: string; label: string }[] = [
